#!/bin/bash
# ROCK 5B network firmware supplement.
...
# Critical files for the WLAN/BT families validated on the ROCK 5B kernel.
required=(
...
[ "$missing" -eq 0 ] || {
    echo "ERROR: required ROCK 5B network firmware is incomplete" >&2
    exit 1
}
