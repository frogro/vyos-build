...
[[ ! -f "$MERGED_ROOT/config/config.boot" ]] ||
...
for F in ap-dhcp-wan-setup.sh dhcp-wan-ssh-setup.sh set-locales.sh modem-connect.sh; do
...
for F in ap-dhcp-wan-setup.sh dhcp-wan-ssh-setup.sh set-locales.sh modem-connect.sh \
...
# Install the new two-stage implementation.
install -D -m 0755 "$SCRIPT_DIR/rock5b-dhcp-wan-firstboot-wrapper.sh" \
    "$MERGED_ROOT/usr/local/sbin/rock5b-dhcp-wan-firstboot-wrapper.sh"
chown root:root "$MERGED_ROOT/usr/local/sbin/rock5b-dhcp-wan-firstboot-wrapper.sh"

install -D -m 0644 "$SCRIPT_DIR/rock5b-dhcp-wan-firstboot.service" \
    "$MERGED_ROOT/etc/systemd/system/rock5b-dhcp-wan-firstboot.service"
install -D -m 0644 "$SCRIPT_DIR/rock5b-dhcp-wan-firstboot.timer" \
...
ln -sfn ../rock5b-dhcp-wan-firstboot.timer \
    "$MERGED_ROOT/etc/systemd/system/timers.target.wants/rock5b-dhcp-wan-firstboot.timer"

# Replace the old custom postconfig hook with a no-op hook.
...
echo "Fertig: zweistufiges DHCP-WAN/SSH-First-Boot-Setup installiert."
