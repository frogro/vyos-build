...
for REQUIRED in config.boot.default rock5b-eth0-firstboot.sh vyos-postconfig-bootup.script; do
    [[ -f "$SCRIPT_DIR/$REQUIRED" ]] || { echo "Pflichtdatei fehlt: $SCRIPT_DIR/$REQUIRED" >&2; exit 1; }
done
...
install -D -m 0750 "$SCRIPT_DIR/vyos-postconfig-bootup.script" "$MERGED_ROOT/opt/vyatta/etc/config/scripts/vyos-postconfig-bootup.script"
install -D -m 0750 "$SCRIPT_DIR/vyos-postconfig-bootup.script" "$MERGED_ROOT/config/scripts/vyos-postconfig-bootup.script"

if [[ -n "$VYATTACFG_GID" ]]; then
    chown 0:"$VYATTACFG_GID" "$MERGED_ROOT/opt/vyatta/etc/config/scripts/vyos-postconfig-bootup.script"
    chown 0:"$VYATTACFG_GID" "$MERGED_ROOT/config/scripts/vyos-postconfig-bootup.script"
else
    chown root:root "$MERGED_ROOT/opt/vyatta/etc/config/scripts/vyos-postconfig-bootup.script"
    chown root:root "$MERGED_ROOT/config/scripts/vyos-postconfig-bootup.script"
fi

grep -q 'rock5b-eth0-firstboot.sh' "$MERGED_ROOT/opt/vyatta/etc/config/scripts/vyos-postconfig-bootup.script"
grep -q 'rock5b-eth0-firstboot.sh' "$MERGED_ROOT/config/scripts/vyos-postconfig-bootup.script"

rm -f "$MERGED_ROOT/persistence.conf" 2>/dev/null || true
echo "Fertig: VyOS-Template und /config-Hook installiert."
